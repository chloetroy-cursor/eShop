...
The script replaces the dedicated sibling worktree `eShop-fe-demo` with a new
branch from `origin/main`, then runs `make demo-reset` inside it. It removes the
previous demo worktree only when it is clean; its branch and PR remain intact.
Never use `git reset` or `git clean`.
