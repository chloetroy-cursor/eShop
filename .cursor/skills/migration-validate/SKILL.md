...
description: Use when validating that a migrated .NET→Rust unit worked — rank evidence honestly, prove blast-radius safety by running code, use the unit harness from plan.md, and decide keep/merge vs do-not-merge vs inconclusive.
---
...
## Artifact ladder (parameterized by service)

Run in order for the unit; skip only with an explicit waiver + reason in the verdict:

1. **Characterization / unit + Rust** — Run the harness command from `plan.md` for this unit (service-specific script like `./scripts/check-catalog.sh` for Catalog, or `dotnet test` on the project that covers the changed domain/API surface **and** `cargo test` for the Rust island). Record exit code and path to evidence (log, CI URL, or saved output).
2. **Service client-style evidence** — If the service has an integration test script or functional suite, run it. If skipped: document why (no Docker, Aspire too heavy, unit not touching the service surface) in `validate.md`.
...
**Keep/merge requirement for migrated units:**
...
1. **Freeze the claim**  
   One sentence: what behavioral guarantee this unit must hold. Example: "{SERVICE} domain rules match pre-migration behavior for the characterized cases when the service uses the Rust-wired path."
...
4. **Walk the artifact ladder**  
   Execute the ladder above for this service. Document exact commands, exit codes, and evidence paths. Use the harness from `plan.md` and direct `cargo test` / parity output — do not invent ad-hoc HTTP probes unless needed. Explicitly record Rust + parity evidence for migrated units.
...
9. **Emit artifact**  
    Write `validate.md` (or append a **Validation** section to the existing `plan.md`). Include commands actually run, evidence level, blast-radius fact status, attempt count if any, Rust/parity evidence (and harness / cargo results from `plan.md`), and the verdict—not aspirational metrics.

## Output: checklist template

```markdown
# Migration validate: {SERVICE} / {UNIT}
...
## Artifact ladder
- [ ] Characterization/unit (harness from plan.md): `{command}` — result: ... — evidence: ...
...
- Works for any baseline → after / unit; parameterized by `SERVICE` from `plan.md`.  
